...
public class XRecyclerView extends RecyclerView {
	...
	public void SetFootViewBackGroundColor(int color){
		LoadingMoreFooter view = (LoadingMoreFooter) mFootViews.get(0);
		view.SetBackgroundColor(color);
	}

	public void SetHeadViewBackGroundColor(int color){
		ArrowRefreshHeader view = (ArrowRefreshHeader) mHeaderViews.get(0);
		view.setBackgroundColor(color);
	}
	...
	public void ScrollforHideFoot(){
		if(getAdapter()!=null){
		Log.e("count:"+getAdapter().getItemCount(),"---");

		smoothScrollToPosition(getAdapter().getItemCount()-3);
		}
	}

	public void ScrollforHideFoot(int position){
		if(getAdapter()!=null){
			Log.e("count:"+getAdapter().getItemCount(),"---"+((LinearLayoutManager)getLayoutManager()).findLastVisibleItemPosition());
	...
	/**
	 * 重置状态
	 * @param isgotop 是否滚至顶端
     */
	public void ResetStatue(boolean isgotop){
		if(isgotop)
		smoothScrollToPosition(0);
		View footView = mFootViews.get(0);
		if (footView instanceof LoadingMoreFooter) {
			((LoadingMoreFooter) footView).setState(3);
		}
		mFootViews.get(0).setVisibility(GONE);
